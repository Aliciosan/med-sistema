...
export default function BookingWizard() {
  ...
  // Lógica de Ocupação: Se cancelado, libera o horário
  const isBusy = (time) => {
    ...
    return allAppointments.some(apt => 
      ...
      apt.time === time &&
      apt.status !== 'cancelled' // <--- IMPORTANTE: Ignora cancelados
    );
  };
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 space-y-8">
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-4 hide-scroll lg:grid lg:grid-cols-7 lg:pb-0">
                        {dates.map((d, i) => (
                            <button
                            key={i}
            ...
            <div className="lg:w-96">
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-lg lg:sticky lg:top-24">
                    ...
                    >
                        {isSaving ? 'Processando...' : <><ShieldCheck size={20} /> Confirmar</>}
                    </button>
                </div>
            </div>
            ...
            <div className={`fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100 lg:hidden transition-transform duration-300 z-50 ${selectedTime ? 'translate-y-0' : 'translate-y-full'}`}>
                ...
                >
                    {isSaving ? '...' : `Confirmar (${selectedTime})`}
                </button>
            </div>
        </div>
      )}
