...
export default function Appointments() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    await fetchAppointments(); // Atualiza a lista na hora
    setIsLoading(false);
  };

  // Formulário Manual (Médico criando agendamento)
  ...
  const handleSave = async (e) => {
    ...
    await appointmentService.create({
      patient_id: 0, // ID genérico para paciente manual
      patient_name: formData.patient,
      doctor_id: 1, // ID do médico logado
      doctor_name: 'Você',
      date: new Date(formData.date).toLocaleDateString('pt-BR'),
      time: formData.time,
      type: formData.type,
      status: 'confirmed' // Médico já cria confirmado
    });
    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Painel de controle de consultas.</p>
        </div>
        <div className="flex w-full md:w-auto gap-2">
          ...
          <button 
            ...
            className="flex-1 md:flex-none bg-primary text-white px-4 py-2.5 rounded-xl font-bold hover:bg-primary-dark transition-all shadow-lg text-sm flex items-center justify-center gap-2"
          >
            <Plus size={18} /> Manual
          </button>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {appointments.map((apt) => (
          <div key={apt.id} className={`bg-white p-5 rounded-3xl shadow-sm border border-slate-200 transition-all ${apt.status === 'cancelled' ? 'opacity-60 grayscale' : ''}`}>
             
             <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-600 flex items-center justify-center font-bold text-lg border border-white shadow-sm">
              ...
              <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase border ${getStatusStyle(apt.status)}`}>
      ...
      {/* MODAL MANUAL */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Adicionar Consulta">
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <button disabled={isLoading} type="submit" className="w-full bg-primary text-white font-bold py-3.5 rounded-xl mt-4 hover:bg-primary-dark">
